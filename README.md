...
curl -sf -F files=@test.html http://localhost:3001/forms/chromium/convert/html -o output.pdf
```

## Features

- **Gotenberg v8** — Chromium + LibreOffice under the hood
- **Docker Compose** — Production and development configs included
...
- **One-line setup** — Full server provisioning script included
...
|----------|---------|-------------|
| `GOTENBERG_PORT` | `3001` | Host port mapping (container always uses 3000) |
...
| `API_TIMEOUT` | `60s` | Maximum time for a single conversion request |
| `API_ROOT_PATH` | `/` | Root path prefix (for reverse proxy setups) |
